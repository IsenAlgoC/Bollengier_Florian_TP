...
int main()
{
	...
	notmask = ~IPv4MaskLenght;//L'inverse du masque pour calculer l'adresse broadcoast


	adresse = w; // On met dans une adresse dans les variables x,y,z,w
	adresse <<= 8;/* décalage de 8 bits à gauche*/
	adresse += x;
	adresse <<= 8;
	adresse += y;
	adresse <<= 8;
	...
	w = adresse;/*On trie pour récuperer les "octets" pour afficher les adresses*/
	w >>= 24;
	...
	printf("Adresse machine : %u.%u.%u.%u\n", w, x, y, z); /* affichage de l'adresse machine */

	adresse_reseau = (adresse) & (IPv4MaskLenght); /* affectation adresse reseau */
	w = adresse_reseau; /* stockage de l adresse réseau dans les variables w,x,y,z */
	w >>= 24;
	...
	printf("L'adresse réseau est: %u.%u.%u.%u\n", w, x, y, z); /* affichage adresse réseau */
	...
	printf("Adresse broadcoast : %u.%u.%u.%u", w, x, y, z); /* affichage de l'adresse broadcast */
