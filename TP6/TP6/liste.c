...
int DeleteLinkedListElem(LinkedList * list, SingleLinkedListElem * item) {
	...
	if (item == list->head) {          // si l'element à supprimer est le premier maillon
		list->head = item->next;      // on redéfinie juste "head" 
		list->size--;
		free(item);                   // puis on libère l'epace et on diminue le nombre d'element 
		return(1);                    // on retourne 1 puisque ça a réussit
	}

	else {                            //sinon
		SingleLinkedListElem* actuel = list->head;   // pointeur sur maillon
		while (actuel->next != item) {                  // on parcourt les maillons jusqu'à trouver celui qui nous interesse
			actuel = actuel->next;
		}
		actuel->next = actuel->next->next;         // une fois trouvé on change le "next" de son maillon précépent vers l'adresson de son suivant
		free(item);                                //on procèce comme dans le premier cas
		list->size--;
		return(1);
	}
